exports.run = async (discord: any, message: any, args: string[]) => {
    ...
    let msg = await message.channel.send(`**Searching youtube** ${discord.getEmoji("gabCircle")}`);

    discord.ytChannelEmbed = async (channelLink: any) => {
        ...
        let youtubeEmbed = discord.createEmbed();
        youtubeEmbed
    ...
    discord.ytPlaylistEmbed = async (playLink: any) => {
    ...
    discord.ytVideoEmbed = async (videoLink: any) => {
    ...
    if (args[1].toLowerCase() === "channel") {
        ytEmbeds = [];
        let channelLink = discord.combineArgs(args, 2);
        if (!channelLink.startsWith("https")) {
            ...
            for (let i = 0; i < length; i++) {
                ...
                await discord.ytChannelEmbed(channelLink);
            }
            discord.createReactionEmbed(ytEmbeds);
            msg.delete(1000);
            return;
        }
        await discord.ytChannelEmbed(channelLink);
    ...
    if (args[1].toLowerCase() === "playlist") {
        ytEmbeds = [];
        let playLink = discord.combineArgs(args, 2);
        if (!playLink.startsWith("https")) {
            ...
            for (let i = 0; i < length; i++) {
                ...
                await discord.ytPlaylistEmbed(playLink);
            }
            discord.createReactionEmbed(ytEmbeds);
            msg.delete(1000);
            return;
    ...
    if (args[1].toLowerCase() === "video") {
        ytEmbeds = [];
        let videoLink = discord.combineArgs(args, 2);
        if (!videoLink.startsWith("https")) {
            ...
            for (let i = 0; i < videoResult.length; i++) {
                ...
                await discord.ytVideoEmbed(videoLink);
            }
            discord.createReactionEmbed(ytEmbeds);
            msg.delete(1000);
            return;
