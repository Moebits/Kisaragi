exports.run = async (client: any, message: any, args: string[]) => {
    ...
    if (args[1] === "images") {
        let board = args[2];
        let query = client.combineArgs(args, 3);
    ...
    client.formatComment = (comment: string, post: string) => {
    ...
    for (let i in posts) {
        ...
        chanEmbed
        .setAuthor("4chan", "https://seeklogo.com/images/1/4chan-logo-620B8734A9-seeklogo.com.png")
        .setTitle(`${posts[0].sub ? posts[0].sub : threads[random].semantic_url} ${client.getEmoji("vigneDead")}`)
        .setURL(url)
        .setImage(posts[i].tim ? `https://i.4cdn.org/${board}/${posts[i].tim}${posts[i].ext}` : url)
        .setDescription(
            `${client.getEmoji("star")}_Post:_ **${url}#p${posts[i].no}**\n` +
            `${client.getEmoji("star")}_Unique IPs:_ **${posts[0].unique_ips}**\n` +
            `${client.getEmoji("star")}_Author:_ **${posts[i].name} ${posts[i].now} No. ${posts[i].no}**\n` +
            `${client.getEmoji("star")}_Image Info:_ ${posts[i].tim ? `File: ${posts[i].filename}${posts[i].ext} (${Math.floor(posts[i].fsize/1024)} KB, ${posts[i].w}x${posts[i].h})`: "None"}\n` +
            `${client.getEmoji("star")}_Comment:_ ${posts[i].com ? client.formatComment(posts[i].com, `${url}#p${posts[i].no}`) : "None"}\n`
        )
